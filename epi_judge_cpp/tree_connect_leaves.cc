...
void inorder(const unique_ptr<BinaryTreeNode<int>> *tree, vector<const unique_ptr<BinaryTreeNode<int>> *> *result);
...
void inorder(const unique_ptr<BinaryTreeNode<int>> *tree, vector<const unique_ptr<BinaryTreeNode<int>> *> *result) {
    if (tree && tree->get()) {
        if (tree->get()->left == nullptr && tree->get()->right == nullptr) {
            result->push_back(tree);
        } else {
            inorder(&(tree->get()->left), result);
            inorder(&(tree->get()->right), result);
        }
    }
}
