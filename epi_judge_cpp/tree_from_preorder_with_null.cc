...
unique_ptr<BinaryTreeNode<int>> helper(const vector<int *> &preorder) {
    int *subtreeKey = preorder[currIndex++];

    if (subtreeKey == nullptr) return nullptr;

    auto left = helper(preorder);
    auto right = helper(preorder);
    return make_unique<BinaryTreeNode<int>>(BinaryTreeNode<int>{*subtreeKey, move(left), move(right)});
}
