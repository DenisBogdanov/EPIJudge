...
void recur(int count, int start, int helper, int finish, vector<vector<int>> &ans) {
    if (count == 0) return;
    recur(count - 1, start, finish, helper, ans);
    ans.push_back({start, finish});
    recur(count - 1, helper, start, finish, ans);
}

vector<vector<int> > ComputeTowerHanoi(int num_rings) {
    if (num_rings == 0) return {};
    vector<vector<int>> ans;
    recur(num_rings, 0, 1, 2, ans);
    return ans;
}
