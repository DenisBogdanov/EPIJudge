...
void DutchFlagPartition(int pivot_index, vector<Color> &colors) {
    int red_idx = 0;
    int blue_idx = colors.size() - 1;
    int i = 0;
    while (i <= blue_idx) {
        if (colors[i] == Color::kRed) {
            std::swap(colors[red_idx], colors[i]);
            i++;
            red_idx++;
        } else if (colors[i] == Color::kWhite) {
            i++;
        } else {
            std::swap(colors[blue_idx], colors[i]);
            blue_idx--;
        }
    }
}
