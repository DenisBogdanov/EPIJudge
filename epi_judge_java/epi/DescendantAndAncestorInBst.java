...
public class DescendantAndAncestorInBst {
    ...
    public static boolean pairIncludesAncestorAndDescendantOfM(BstNode<Integer> possibleAncOrDesc0,
        ...
        return (isDescendant(possibleAncOrDesc0, middle) && isDescendant(middle, possibleAncOrDesc1))
                || (isDescendant(middle, possibleAncOrDesc0) && isDescendant(possibleAncOrDesc1, middle));
    }

    private static boolean isDescendant(BstNode<Integer> candidate, BstNode<Integer> node) {
        if (node == null) return false;
        if (candidate.data < node.data) {
            return isDescendant(candidate, node.left);
        } else if (candidate.data > node.data) {
            return isDescendant(candidate, node.right);
        } else {
            return true;
        }
    }
