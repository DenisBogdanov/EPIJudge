...
public class AddingCredits {
    ...
    public static class ClientsCreditsInfo {
        private final Map<String, Integer> idToClientMap = new HashMap<>();
        private final TreeMap<Integer, Set<String>> creditToClientsMap = new TreeMap<>();
        private int addedToEveryone = 0;
        ...
        public void insert(String clientID, int c) {
            ...
            idToClientMap.put(clientID, c);
            creditToClientsMap.computeIfAbsent(c, k -> new HashSet<>())
                    .add(clientID);
        }

        public boolean remove(String clientID) {
            Integer credits = idToClientMap.remove(clientID);
            if (credits == null) return false;

            Set<String> clients = creditToClientsMap.get(credits);
            if (clients.size() == 1) {
                creditToClientsMap.remove(credits);
            } else {
        ...
        public int lookup(String clientID) {
            Integer credits = idToClientMap.get(clientID);
            if (credits == null) return -1;
            return credits + addedToEveryone;
        }
        ...
        public String max() {
            return creditToClientsMap.isEmpty() ? "" : creditToClientsMap.lastEntry().getValue().iterator().next();
        }
    }
