...
public class DutchNationalFlag {
    ...
    public static void dutchFlagPartition(int pivotIndex, List<Color> colors) {
        ...
        int ltIdx = 0;
        int eqIdx = 0;
        int gtIdx = colors.size();

        while (eqIdx < gtIdx) {
            if (colors.get(eqIdx).ordinal() < pivot.ordinal()) {
                Collections.swap(colors, ltIdx++, eqIdx++);
            } else if (colors.get(eqIdx).ordinal() == pivot.ordinal()) {
                eqIdx++;
            } else {
                Collections.swap(colors, eqIdx, --gtIdx);
            }
        }
    }
