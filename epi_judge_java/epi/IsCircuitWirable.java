...
public class IsCircuitWirable {
    ...
    public static boolean isAnyPlacementFeasible(List<GraphVertex> graph) {
        for (GraphVertex graphVertex : graph) {
            if (graphVertex.d != -1) continue;
            graphVertex.d = 0;

            if (!isPossible(graphVertex)) {
                return false;
            }
        }
    ...
    private static boolean isPossible(GraphVertex graphVertex) {
        int d = graphVertex.d;
        for (GraphVertex neighbour : graphVertex.edges) {
            if (neighbour.d == d) return false;
            if (neighbour.d == -1) {
                neighbour.d = 1 - d;
                if (!isPossible(neighbour)) {
                    return false;
                }
            }
        }
