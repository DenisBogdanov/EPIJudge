...
public class IsCircuitWirable {
    ...
    public static boolean isAnyPlacementFeasible(List<GraphVertex> graph) {
        for (GraphVertex graphVertex : graph) {
            if (graphVertex.d == -1 && !paintDfsPossible(graphVertex, 0)) {
                return false;
            }
        }
    ...
    private static boolean paintDfsPossible(GraphVertex graphVertex, int baseColor) {
        graphVertex.d = baseColor;
        for (GraphVertex neighbour : graphVertex.edges) {
            if (neighbour.d == baseColor) return false;

            if (neighbour.d == -1) {
                if (!paintDfsPossible(neighbour, 1 - baseColor)) {
                    return false;
                }
            }
        }
