...
public class IntervalsUnion {
    public static List<Interval> unionOfIntervals(List<Interval> intervals) {
        ...
        intervals.sort((i1, i2) -> {
            if (i1.left.val < i2.left.val) return -1;
            if (i1.left.val > i2.left.val) return 1;

            if (i1.left.isClosed) return -1;
            if (i1.right.isClosed) return 1;
            return 0;
        });
        ...
        for (int i = 1; i < intervals.size(); i++) {
            ...
            if (candidate.left.val == candidate.right.val && !candidate.left.isClosed) continue;

            if (curr.right.val > candidate.left.val) {
                if (curr.right.val == candidate.right.val) {
                    curr.right.isClosed = curr.right.isClosed || candidate.right.isClosed;
                } else if (curr.right.val < candidate.right.val) {
                    curr.right = candidate.right;
                }
            } else if (curr.right.val < candidate.left.val) {
                result.add(curr);
                curr = candidate;
            } else {
                if (curr.right.isClosed || candidate.left.isClosed) {
                    curr.right = candidate.right;
                } else {
                    result.add(curr);
                    curr = candidate;
