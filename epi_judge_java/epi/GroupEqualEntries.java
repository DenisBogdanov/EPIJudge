...
public class GroupEqualEntries {
    ...
    public static void groupByAge(List<Person> people) {
        Map<Integer, List<Person>> ageToPeopleMap = new HashMap<>();
        for (Person person : people) {
            ageToPeopleMap.computeIfAbsent(person.age, k -> new ArrayList<>())
                    .add(person);
        }

        people.clear();
        for (List<Person> peopleOfSameAge : ageToPeopleMap.values()) {
            people.addAll(peopleOfSameAge);
        }
    }
