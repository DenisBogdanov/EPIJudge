...
public class GroupEqualEntries {
    ...
    public static void groupByAge(List<Person> people) {
        int[] ageToCount = new int[10_000];
        for (Person person : people) {
            ageToCount[person.age]++;
        }

        for (int i = 1; i < ageToCount.length; i++) {
            ageToCount[i] += ageToCount[i - 1];
        }

        Person[] result = new Person[people.size()];
        for (Person person : people) {
            int index = --ageToCount[person.age];
            result[index] = person;
        }

        for (int i = 0; i < result.length; i++) {
            people.set(i, result[i]);
        }
    }
