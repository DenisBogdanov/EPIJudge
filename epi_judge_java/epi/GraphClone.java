...
public class GraphClone {
    ...
    public static GraphVertex cloneGraph(GraphVertex graph) {
        ...
        return clone(graph, oldVertexToNewVertexMap);
    }

    private static GraphVertex clone(GraphVertex graph, Map<GraphVertex, GraphVertex> oldVertexToNewVertexMap) {
        GraphVertex clonedVertex = oldVertexToNewVertexMap.get(graph);
        if (clonedVertex != null) return clonedVertex;
        clonedVertex = new GraphVertex(graph.label);
        oldVertexToNewVertexMap.put(graph, clonedVertex);

        for (GraphVertex neighbour : graph.edges) {
            clonedVertex.edges.add(clone(neighbour, oldVertexToNewVertexMap));
        }
