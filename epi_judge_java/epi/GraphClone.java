...
public class GraphClone {
    ...
    public static GraphVertex cloneGraph(GraphVertex graph) {
        ...
        copyDfs(graph, oldVertexToNewVertexMap);
        return oldVertexToNewVertexMap.get(graph);
    }

    private static GraphVertex copyDfs(GraphVertex vertex, Map<GraphVertex, GraphVertex> oldVertexToNewVertexMap) {
        if (oldVertexToNewVertexMap.containsKey(vertex)) return oldVertexToNewVertexMap.get(vertex);

        GraphVertex copy = new GraphVertex(vertex.label);
        oldVertexToNewVertexMap.put(vertex, copy);

        for (GraphVertex neighbour : vertex.edges) {
            copy.edges.add(copyDfs(neighbour, oldVertexToNewVertexMap));
        }
