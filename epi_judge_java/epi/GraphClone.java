...
public class GraphClone {
    ...
    public static GraphVertex cloneGraph(GraphVertex graph) {
        ...
        GraphVertex clonedVertex = new GraphVertex(graph.label);
        oldVertexToNewVertexMap.put(graph, clonedVertex);

        cloneDfs(graph, clonedVertex, oldVertexToNewVertexMap);

        return clonedVertex;
    }

    private static void cloneDfs(GraphVertex graph, GraphVertex clonedVertex, Map<GraphVertex, GraphVertex> oldVertexToNewVertexMap) {
        for (GraphVertex neighbour : graph.edges) {
            GraphVertex clonedNeighbour = oldVertexToNewVertexMap.get(neighbour);
            if (clonedNeighbour == null) {
                clonedNeighbour = new GraphVertex(neighbour.label);
                oldVertexToNewVertexMap.put(neighbour, clonedNeighbour);
                cloneDfs(neighbour, clonedNeighbour, oldVertexToNewVertexMap);
            }

            clonedVertex.edges.add(clonedNeighbour);
        }
    }
