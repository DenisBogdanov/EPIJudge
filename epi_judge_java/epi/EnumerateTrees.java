    ...
    public static List<BinaryTreeNode<Integer>> generateAllBinaryTrees(int numNodes) {
        ...
        if (numNodes == 0) {
            ...
            for (int i = 0; i < numNodes; i++) {
                ...
                List<BinaryTreeNode<Integer>> right = generateAllBinaryTrees(numNodes - i - 1);

                for (BinaryTreeNode<Integer> fromLeft : left) {
                    for (BinaryTreeNode<Integer> fromRight : right) {
                        BinaryTreeNode<Integer> root = new BinaryTreeNode<>(1);
                        root.left = fromLeft;
                        root.right = fromRight;
