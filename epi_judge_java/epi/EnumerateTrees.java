...
public class EnumerateTrees {
    ...
    public static List<BinaryTreeNode<Integer>> generateAllBinaryTrees(int numNodes) {
        ...
        for (int leftNodesCount = 0; leftNodesCount < numNodes; leftNodesCount++) {
            int rightNodesCount = numNodes - 1 - leftNodesCount;

            List<BinaryTreeNode<Integer>> leftSubtrees = generateAllBinaryTrees(leftNodesCount);
            List<BinaryTreeNode<Integer>> rightSubtrees = generateAllBinaryTrees(rightNodesCount);

            for (BinaryTreeNode<Integer> leftSubtree : leftSubtrees) {
                for (BinaryTreeNode<Integer> rightSubtree : rightSubtrees) {
                    result.add(new BinaryTreeNode<>(0, leftSubtree, rightSubtree));
                }
            }
        }
