...
public class SudokuSolve {
    ...
    private static boolean helper(List<List<Integer>> partialAssignment, int r, int c) {
        if (c == SIZE) {
            r++;
            c = 0;
        ...
        if (partialAssignment.get(r).get(c) != 0) return helper(partialAssignment, r, c + 1);

        for (int candidate = 1; candidate <= SIZE; candidate++) {
            partialAssignment.get(r).set(c, candidate);
            if (IsValidSudoku.isValidSudoku(partialAssignment)) {
                if (helper(partialAssignment, r, c + 1)) {
                    return true;
                }
            }
        }

        partialAssignment.get(r).set(c, 0);
        return false;
    }
