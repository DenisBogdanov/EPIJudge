...
public class AlternatingArray {
    public static void rearrange(List<Integer> list) {
        boolean nextShouldBeLessOrEq = false;

        for (int i = 1; i < list.size(); i++) {
            if (list.get(i) > list.get(i - 1) && nextShouldBeLessOrEq) {
                Collections.swap(list, i, i - 1);
            } else if (list.get(i) < list.get(i - 1) && !nextShouldBeLessOrEq) {
                Collections.swap(list, i, i - 1);
            }

            nextShouldBeLessOrEq = !nextShouldBeLessOrEq;
        }
    }
