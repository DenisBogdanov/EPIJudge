...
public class LruCache {
    private final LinkedHashMap<Integer, Integer> isbnToPriceMap;

    LruCache(final int capacity) {
        isbnToPriceMap = new LinkedHashMap<Integer, Integer>(capacity, 0.75f, true) {
            @Override
    ...
    public Integer lookup(Integer key) {
        Integer price = isbnToPriceMap.remove(key);
        if (price != null) {
            isbnToPriceMap.put(key, price);
            return price;
        }
    ...
    public void insert(Integer key, Integer value) {
        Integer oldPrice = isbnToPriceMap.remove(key);
        if (oldPrice != null) {
            isbnToPriceMap.put(key, oldPrice);
        } else {
            isbnToPriceMap.put(key, value);
        }
    }

    public Boolean erase(Object key) {
        if (isbnToPriceMap.isEmpty()) return false;
        Integer oldPrice = isbnToPriceMap.remove(key);
